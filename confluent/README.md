...
```mermaid
flowchart LR
  gh["GitHub Source connector"] -->|"04_incidents_from_github.sql"| inc["tribunal.incidents"]
...
10. **Connector:** cluster → **Connectors** → **Add connector** → **GitHub Source**.
    - Create a GitHub personal access token (classic, `public_repo` read-only is enough) at github.com/settings/tokens.
    ...
    - Topic prefix: `github-`, so the topic is `github-issues`. Output format: **JSON_SR**. Tasks: **1**.
    - Launch. When `github-issues` → Messages shows records, run `DESCRIBE \`github-issues\`;` in the workspace.
      Fix the marked column-mapping block in `04_incidents_from_github.sql` if the names differ, then run it and leave it **running**.
...
When you're done: pause the connector, stop the Flink statements, then delete the `tribunal` environment.
